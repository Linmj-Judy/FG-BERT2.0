...
def load_dataset(task, batch_size, label, seed):
    
    graph_dataset = Graph_Classification_Dataset(task + '.csv', smiles_field='canonical_smiles',
...
def hy_main(args):
    ...
    for seed in [1231]: #可以做k折交叉验证
        ...
        val_auc, test_auc, a_list, test_y_t, test_y_p = main(task, label, seed, args, FTmodel, pretraining=True)
        # 更新包装器
